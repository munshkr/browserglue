...
const buildRPCServer = (server: Server) => {
  ...
  rpcServer.addMethod(
    "addChannel",
    ({ path, port, sendPort }: AddChannelParams) => {
  ...
  rpcServer.addMethod(
    "unsubscribePort",
    ({ path, port }: UnsubscribePortParams) => {
  ...
  rpcServer.addMethod(
    "unsubscribeAllPorts",
    ({ path }: UnsubscribeAllPortsParams) => {
...
class Server {
  ...
  constructor(
    { host, port }: ServerOptions = { host: "localhost", port: DEFAULT_PORT }
  ) {
    this.host = host;
    this.port = port;
  ...
  unsubscribePort(path: string, port: number): boolean {
    ...
    const newPorts = this._channels[path].subscribedPorts.filter(
      (p) => p !== port
    );
  ...
  protected _emitServerChangeEvent(): void {
    this._emitServerEvent("change", this._channels);
  }

  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  protected _emitServerEvent(event: string | symbol, message?: any): void {
